...
def compute_ves_by_diff(exec_results,diff_json_path):
    ...
    for i,content in enumerate(contents):
        if i >= num_queries:  # Avoid out-of-bounds indexing
            print(f"Skipping index {i}, as exec_results has only {num_queries} elements.")
            continue
...
if __name__ == '__main__':
    """
    This script was taken from BIRD github repository. [https://github.com/AlibabaResearch/DAMO-ConvAI/tree/main/bird]
    
    Some updates in the script:
    - Added a check in the calculate VES by difference:
        if i >= num_queries: # Avoid out-of-bounds indexing
            print(f"Skipping index {i}, as exec_results has only {num_queries} elements.")
            continue
    """
