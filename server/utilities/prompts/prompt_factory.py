...
class PromptFactory:
    ...
    def get_prompt_class(prompt_type: PromptType, target_question: str, examples=None, shots=None, schema_format=None, matches = {}) -> str:
        if prompt_type == PromptType.BASIC:
            ...
            return FullInformationOrganizationPrompt(shots=shots, target_question=target_question, schema_format=schema_format).get_prompt(matches=matches)
            ...
            return SemanticAndFullInformationOrganizationPrompt(shots=shots, target_question=target_question, schema_format=schema_format).get_prompt(matches=matches)
        else:
